...
class rubros_control{
//#region OBTENER
    async ObtenerTotalRubros(req:Request,res:Response){
        try {
    ...
    async ObtenerRubros(req:Request, res:Response){
        try {
            //Obtengo los datos de paginacion y filtros
            const filtroTabla = req.body; 
    ...
    ObtenerRubro(req:Request, res:Response){
        try {
            ...
            pool.getConnection(function(error, connection) {
                ...
                connection.query(query, rubro, function (error, fields) {
                    connection.release();
    ...
    ObtenerCoincidencias(req:Request, res:Response){
        try {
            const rubro = req.params.rubro;
            const query = `SELECT COUNT(id) total FROM rubros WHERE idRubro = ?`; 
...
//#region ABM
    Agregar(req:Request, res:Response){
        try {
            ...
            let parametros:any = [rubro.nombre];
            const query = `INSERT INTO rubros (nombre) VALUES (?) `
    ...
    Modificar(req:Request, res:Response){
        try {
            const rubro = req.body;
            let parametros:any = [rubro.nombre, rubro.id];
            const query = ` UPDATE rubros SET nombre = ?
                            WHERE id = ? `
                ...
                connection.beginTransaction(function(error) {
                    if (error) {//ERROR AL INICIAR LA TRANSACCION (Rollback y Liberar conexion)
                        ...
                        if(data.condicion=="baja"){
                            //#region ACTUALIZAR PRODUCTO/RUBRO
                            //Actualiza los productos que contienen el id del rubro a eliminar, y les asigna el numero 1 (Eliminado)
                            let sql = `UPDATE productos
                                       SET idRubro = 1 
                                       WHERE idRubro = ? `;
                            connection.query(sql, data.rubro, function (error, fields) {
                        ...
                        let query = `DELETE FROM rubros
                                     WHERE id = ? `;
                        connection.query(query, data.rubro, function (error, fields) {
                            if (error) {//ERROR EN QUERY (Rollback y Liberar conexion)
                                connection.rollback(function() {
                                    connection.release();
                                    HandlearError(req,res,"db",error);
...
// Obtiene una query de rubros y el total de registros si se requiere / Usado para paginación
...
export const rubrosctrl = new rubros_control();